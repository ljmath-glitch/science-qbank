...
**Step 2 選題**（`wzRenderList`）：左側題目卡片＋右側 `#wzMonitor` **即時監控／範圍導覽**（`wzRenderMonitor`）：已選題數、難易度分布長條，加上 **各範圍進度＝大單元→子單元→知識點 三層可收合樹**（`WZ_MONEXPANDED` 記展開節點，`wzMonToggle(i)` 切收合），每層顯示「已選/可用」與 ✔（已選過）/○（未選）。點任一層名稱 `wzSetUnitFilter(i)` 把題目清單下鑽到該範圍：`WZ_UNITFILTER` 是路徑前綴（`book\u0001unit[\u0001sub[\u0001kp]]`），`wzRenderList` 用 `_wzPath(it)`（`book\u0001unit\u0001sub\u0001kp`）做前綴比對；清單頂端出現「目前只看…✕看全部範圍」橫幅。解決大範圍時茫茫大海撈針。卡片：checkbox 選題（`wzPick`→`togglePick`，連動題組）、meta 列（難易度徽章＋冊/單元/子單元/題型）、簡化題幹（`_wzSimple` 用 `stripRichMarks` 去掉 `{{...}}` 富文字標記＋把 `$…$` 換 ▢）、`✎ 編輯` 鈕（`wzEdit`→ 直接呼叫題庫管理的 `openEdit(id)`，`saveEdit` 存檔後會刷新精靈清單）。點卡片內文 `wzToggleExpand` 展開/收合**完整題目**（`_wzFullHtml`：題組文章＋題幹＋圖＋答案＋詳解，`renderMath` 渲染 LaTeX）。有題型/難易度下拉與關鍵字篩選、待選/已選分頁。「智慧組卷」鈕在精靈內以 `WZ_SCOPE` 為題源（`_mixSelectablePool` 偵測 `#composer` 未隱藏時改用 `wzInScope`）。
